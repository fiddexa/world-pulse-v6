...
def render_section_page(
    ...
    """
    Render a dense adaptive newspaper section page.

    The page receives its own EditionPage object and therefore renders
    only the events assigned to that page.

    Layout adapts to the number of stories:
      1 story  -> large lead story
      2 stories -> two-column layout
      3 stories -> lead + two compact stories
      4 stories -> 2x2 grid
      5+ stories -> dense two-column newspaper grid
    ...
    def _draw_story_card(
        ...
        *,
        ...
        if lead:
            image_height = min(
                270,
                max(150, int(card_height * 0.42)),
            )
        else:
            image_height = min(
                185,
                max(105, int(card_height * 0.38)),
            )
        ...
        draw.text(
            (x, current_y),
            story_number,
            font=_font(13 if not lead else 15, bold=True),
            fill=RED,
        )

        current_y += 23 if lead else 20
    ...
    if event_count == 0:
        ...
        # ------------------------------------------------------------
        # THREE STORIES: large lead + two compact stories
        # ------------------------------------------------------------
        ...
        # dense two-column newspaper grid.
        #
        # We intentionally do not use a fixed 330px row height.
        # The available editorial area is divided into rows so that
        # all assigned stories remain visible on the page.
        # ------------------------------------------------------------

        columns = 2

        rows = (
            event_count + columns - 1
        ) // columns
        ...
        for index, event in enumerate(events):
            ...
            _draw_story_card(
                ...
                index,
                lead=(index == 0 and event_count <= 4),
            )
