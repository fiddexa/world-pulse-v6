...
def _card_height(event: dict) -> int:
    """
    Calculate card height from the actual rendered content.

    The same wrapping limits used by the renderer are respected, so
    title, summary and source text remain inside the card boundary.
    """

    if not isinstance(event, dict):
        return 200

    probe = Image.new("RGB", (WIDTH, MOBILE_PAGE_HEIGHT), NEWSPAPER)
    ...
    # Keep compact cards for short stories, but never allow the
    # rendered content to exceed the border.
    return max(200, min(520, height))
