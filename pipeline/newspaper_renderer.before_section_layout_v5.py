...
def render_section_page(
    ...
    """
    Render a content-driven newspaper section page.

    The layout is based on the amount of editorial content rather
    than stretching cards to fill the entire page.

    1 story  -> dominant article
    2 stories -> two newspaper columns
    3 stories -> lead + two secondary columns
    4+ stories -> compact newspaper grid
    """
    ...
    def _source(event: dict) -> str:
        return _safe_text(
            ...
            or _content(event).get("source")
            or _content(event).get("source_name")
        )

    def _story_height(event: dict, width: int, lead: bool) -> int:
        ...
        # Approximate line counts from the actual text.
        title_chars = max(
            1,
            int(width / (
                25 if lead else 31
        ...
        if _image_path(event):
            height += 180 if lead else 105
        ...
        return max(
            250 if lead else 205,
            min(
                height,
                620 if lead else 460,
            ),
        )
    ...
    if event_count == 0:
        ...
        # THREE STORIES
        #
        # Large lead column + two smaller columns.
        # ------------------------------------------------------------
        ...
        #
        # Compact newspaper grid. Do not stretch stories to footer.
        # ------------------------------------------------------------
    ...
    # DAILY BRIEF remains footer-only.
    draw.text(
        (
            MARGIN,
            footer_top + 88,
